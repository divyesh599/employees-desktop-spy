...
if not os.path.isfile(startup + '/' + 'desktopspy_server_v01.lnk'):
    winshell.CreateShortcut(
        ...
        Target=r"C:\Program Files (x86)\desktop-spy-server\desktopspy_server_v01.exe",
        Icon=(r"C:\Program Files (x86)\desktop-spy-server\desktopspy_server_v01.exe", 0),
...
def employee_thread(client_conn):  # main thread that separates employee
    ...
    def rcv_data():  # receive msg thread
        try:
            ...
            pass
            # messagebox.showerror('Error', str(temp))
        return 0
    ...
    entry_box = Text(chatbox, padx=10, width=25, height=3, bg='#d9e6fc', font=font_temp)
...
#################################################################################
# Extra Argument --
# bg_temp = "#4285F4" #Google
# bg_temp = "#006491" #Domino’s
# bg_temp = "#007DB8" #Dell
bg_temp = '#DDDDDD'
